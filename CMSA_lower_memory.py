...
class Synapseslayer:
    ...
    def update(self, input: torch.Tensor, potential:torch.Tensor):
        ...
        self.i = torch.einsum('abij, ij-> abij', input, self.weight)*(self.Vrest - potential).unsqueeze(2).unsqueeze(3).repeat(1,1,self.shape[2],self.shape[3])  # 将单个神经元的膜电位展开成和突触张量同阶数的张量
        return self.i
...
class Network:
    ...
    def update(self, inputE:torch.Tensor, inputI:torch.Tensor):
        E_potential, E_spike, E_decay = self.E_neurons.update(inputE+self.synapsesEE.i.sum(dim=(2,3))+self.synapsesIE.i.sum(dim=(2,3)))
        I_potential, I_spike, I_decay = self.I_neurons.update(inputI+self.synapsesII.i.sum(dim=(2,3))+self.synapsesEI.i.sum(dim=(2,3)))
